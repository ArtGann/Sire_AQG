...
export async function onRequest({ request, env = {} } = {}) {
  try {
    ...
    const ghl = {
      ...
      gutter_size: input.size, gutter_type: input.size ? `${input.size}\" Aluminum K-Style` : "", gutter_mode: input.gutterMode, gutter_lf: input.gutterLf, gutter_lf_source: input.gutterMode === "whole_house" && !input.gutterLf ? "Estimated from home size" : input.gutterLf ? "Provided by customer" : "",
      guard_type: input.guardType, gutter_guards: input.guardType, guard_mode: input.guardMode, guard_lf: input.guardLf, guard_lf_source: input.guardMode === "same_gutter" ? "Based on gutter system length" : input.guardLf ? "Provided by customer" : "",
      fascia_mode: input.fasciaMode, fascia_lf: input.fasciaLf, fascia_lf_source: input.fasciaLf ? "Provided by customer" : "", soffit_mode: input.soffitMode, soffit_lf: input.soffitLf, soffit_lf_source: input.soffitLf ? "Provided by customer" : "",
      home_stories: input.stories || "", square_feet: input.squareFeet || "", downspout_count: input.downspoutCount, downspout_length_per_unit: input.downspoutLengthPerUnit || (input.stories ? input.stories * 10 : ""), downspout_total_lf: input.downspoutCount * (input.downspoutLengthPerUnit || (input.stories ? input.stories * 10 : 0)), elbow_count: input.elbowCount, connector_count: input.connectorCount, downspout_connector_count: input.connectorCount, miter_count: input.miterCount, gutter_miter_count: input.miterCount,
      ...
      sms_consent: lead.sms_consent, sms_consent_timestamp: lead.sms_consent ? now : "", sms_consent_text_version: lead.sms_consent ? "2026-07-11" : "", idempotency_key: lead.idempotency_key, request_ip: lead.request_ip, user_agent: lead.user_agent, page_form_source: "Website Estimate Form"
    };
